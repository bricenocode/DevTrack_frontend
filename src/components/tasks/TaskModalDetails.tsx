            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-neutral-300  text-left align-middle shadow-xl transition-all p-16">
                  <p className='text-sm text-slate-700'>Agregada el:  {formatDate(data.createdAt)} </p>

                  <p className='text-sm text-slate-700'>Última actualización: {formatDate(data.updatedAt)}</p>
                  ...
                  <p className='text-lg text-slate-700 mb-2'>Descripción: {data.description}</p>
                  {data.completedBy && (
                    <>
                      <p className='font-bold text-2xl text-slate-900 my-5'> Historial de cambios: </p>
                      <ul className='list-decimal'>
                        {data.completedBy.map((activityLog) => (
                          <li key={activityLog._id}>
                            <span className='font-boold text-slate-700'>
